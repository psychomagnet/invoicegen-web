...
export default function App() {
  ...
  const [folderId, setFolderId] = useState(
    localStorage.getItem(LS_KEYS.folderId) || ""
  );
  const [ccEmail, setCcEmail] = useState(
    localStorage.getItem(LS_KEYS.ccEmail) || ""
  );
  ...
  // Init auth client when clientId appears
  useEffect(() => {
    if (!clientId) return;
    try {
  ...
  // After auth + folder set, load folder + history
  useEffect(() => {
    async function load() {
  ...
  // Gate: show Login until truly ready
  const token = getAccessToken();
  if (!token || !clientId || !folderId || !authed) {
    return <Login onAuthed={onAuthedFromLogin} />;
  }

  // Helper to compute due date +15d from invoiceDate
  function dueFromInvoiceDate() {
    const [dd, mm, yyyy] = invoiceDate.split("/").map(Number);
      ...
      <div className="layout">
        ...
        <aside className="panel" style={{ display: historyOpen ? "block" : "none" }}>
          ...
          >
            ...
            <button className="btn btn-small" onClick={() => setHistoryOpen(false)}>
              Hide
            </button>
          </div>
          {history.length === 0 ? (
        ...
        <main className="panel">
          ...
          >
            <h2>Invoice</h2>
            {!historyOpen && (
              <button className="btn btn-small" onClick={() => setHistoryOpen(true)}>
                Show history
              </button>
            )}
          </div>
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setItems((prev) =>
                      prev.map((p) =>
                        p.id === it.id ? { ...p, description: e.target.value } : p
                      )
                    )
                  }
                />
                <input
                  ...
                  onChange={(e) =>
                    setItems((prev) =>
                      prev.map((p) =>
                        p.id === it.id ? { ...p, qty: Number(e.target.value) } : p
                      )
                    )
                  }
                />
                <input
                  ...
                  onChange={(e) =>
                    setItems((prev) =>
                      prev.map((p) =>
                        p.id === it.id ? { ...p, rate: Number(e.target.value) } : p
                      )
                    )
                  }
                />
          ...
          <div className="preview" style={{ marginTop: 12 }}>
            <div>
              {invoiceDate} →{" "}
              <span style={{ color: "var(--muted)" }}>due {dueFromInvoiceDate()}</span>
            </div>
            <div>
              Invoice{" "}
              <span
            ...
            <div style={{ marginTop: 8 }}>
              <div className="help" style={{ textTransform: "uppercase" }}>
                Billed to
              </div>
              <div>
        ...
        >
          ...
          >
            <div style={{ fontWeight: 600, marginBottom: 6 }}>Numbering</div>
            <div className="help" style={{ marginBottom: 6 }}>
              Next #:{" "}
              <span style={{ fontFamily: "monospace" }}>{nextNumberLabel}</span>
            </div>
            <label
              className="help"
              style={{ display: "flex", alignItems: "center", gap: 6 }}
            >
              <input
                ...
                onChange={(e) =>
                  persistNumbering({ ...numRules, resetEachYear: e.target.checked })
                }
              />
          ...
          <div
            ...
            style={{ padding: 12, margin: "12px 0" }}
          >
            <div style={{ fontWeight: 600, marginBottom: 6 }}>Quick contacts</div>
            <button
              className="btn btn-small"
              onClick={() =>
                persistContacts([
                  ...contacts,
                  { name: "", address1: "", address2: "", cityzip: "" },
                ])
              }
            >
              Add contact
                ...
                <div key={idx} className="panel" style={{ padding: 8 }}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      persistContacts(
                        contacts.map((x, i) =>
                          i === idx ? { ...x, address1: e.target.value } : x
                        )
                      )
                    }
                  />
                  <input
                    ...
                    onChange={(e) =>
                      persistContacts(
                        contacts.map((x, i) =>
                          i === idx ? { ...x, address2: e.target.value } : x
                        )
                      )
                    }
                  />
                  <input
                    ...
                    onChange={(e) =>
                      persistContacts(
                        contacts.map((x, i) =>
                          i === idx ? { ...x, cityzip: e.target.value } : x
                        )
                      )
                    }
                  />
                  <div style={{ textAlign: "right", marginTop: 6 }}>
                    <button
                      className="btn btn-small"
                      onClick={() =>
                        persistContacts(contacts.filter((_, i) => i !== idx))
                      }
                    >
                      Delete
                    </button>
